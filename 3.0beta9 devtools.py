...
class SplashScreen(QSplashScreen):
    ...
    def __init__(self):
        ...
        icon_path = os.path.join("icons", "favi.png")
        icon_pixmap = QPixmap(icon_path).scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation) if os.path.exists(icon_path) else QPixmap(100, 100)
...
class Sidebar(QWidget):
    def __init__(self, parent=None):
        ...
        self.setMinimumWidth(300)  # 减小最小宽度
        self.setMaximumWidth(400)  # 减小最大宽度
...
class HistoryDialog(QDialog):
    def __init__(self, history_data, parent=None):
        ...
        self.setWindowIcon(QIcon(os.path.join("icons", "recent.png")))
...
class MainWindow(QMainWindow):
    def __init__(self, splash=None):
        ...
        self.setWindowIcon(QIcon(os.path.join('icons', 'favi.png')))
    ...
    def toggle_dev_tools(self):
        ...
        if hasattr(current_webview, 'dev_tools_view'):
            # 如果开发者工具已打开，则关闭
            current_webview.dev_tools_view.close()
            delattr(current_webview, 'dev_tools_view')
        else:
            # 如果开发者工具未打开，则创建并显示
            dev_tools_view = QWebEngineView()
            current_webview.page().setDevToolsPage(dev_tools_view.page())
            dev_tools_view.setWindowTitle(f"开发者工具 - {current_webview.page().title()}")
            dev_tools_view.resize(800, 600)
            dev_tools_view.show()
            current_webview.dev_tools_view = dev_tools_view
    ...
    def init_toolbar(self):
        ...
        # 导航按钮
        self.back_btn = QAction(QIcon(os.path.join('icons', 'back.png')), '返回', self)
        self.forward_btn = QAction(QIcon(os.path.join('icons', 'forward.png')), '前进', self)
        self.refresh_btn = QAction(QIcon(os.path.join('icons', 'refresh.png')), '刷新', self)
        self.home_btn = QAction(QIcon(os.path.join('icons', 'home.png')), '主页', self)
        self.ai_btn = QAction(QIcon(os.path.join('icons', 'ai.png')), 'Horn.Browser Bar', self)
        self.history_btn = QAction(QIcon(os.path.join('icons', 'recent.png')), '历史记录', self)
        ...
        self.external_btn.setIcon(QIcon(os.path.join('icons', 'openinother.png')))
